...
>
> 주소는 두 줄입니다. **`thumbnail-url:`** 은 화면용 저화질(500px 안쪽)이라 수백 종짜리 주제도 가볍게 열리고,
> **`podium-url:`** 은 포스터용 원본입니다.
...
| 옐로우미플 · 데블다이스 | 각 20~40 |
| **BggTop300_260805** | 300 (BGG 종합 순위 1~300위, 제목은 한국어판 표기) |
...
> 폴더에 `info.md` 가 이미 들어 있으면 뼈대 대신 그 파일을 그대로 씁니다 — 그림 없이 `info.md` 만 든 폴더도 주제가 됩니다.
>
...
| 기호 | 뜻 |
|---|---|
| `# 파일명` | 아이템 하나. `topics/images/` 의 파일명과 정확히 같아야 합니다. 확장자를 빼고 `# 신라면` 이라고만 적어도 되며, 그때는 `신라면.png` 처럼 이름이 같은 그림을 찾아 씁니다 |
| `# global` | 모든 아이템 설명 맨 위에 붙습니다 |
| `-` / `+` | 필수 설명(어디서나) / 추가 설명(List 에서만) |
| `[ ]` | 어디에나 넣을 수 있는 체크박스 |
| `//` | 주석 — 앱은 읽지 않습니다 |

**그림 파일이 없어도 됩니다.** 그때는 이름만 적힌 카드가 나옵니다 — 이름만 줄줄이 적은 `info.md` 도 그대로 순위표가 됩니다. 파일 대신 웹 주소를 쓸 수도 있는데, 항목이 아주 많을 때 쓰는 방법이라 [그림을 주소로 붙이기](docs/image-url.md) 에 따로 정리했습니다.
