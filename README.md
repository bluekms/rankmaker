...
| 보기 전환 | List / Gallery / Tier 버튼. 액자형은 "Cols" 입력으로 열 수 조절 |
| 티어표 만들기 | Tier 보기 — 아이템을 행으로 드래그하면 같은 티어(공동 순위), 행 사이 틈에 놓으면 새 티어. 티어는 최대 5개이며 나머지는 맨 아래 풀 행에 모입니다. 라벨 클릭으로 이름 변경 |
| 테마 전환 | 우상단 🌙 / ☀️ |
