...
- **드래그&드롭으로 순위 조작** — 티어표, 한글 초성 검색, 리스트/갤러리/티어 보기
...
| 티어표 만들기 | Tier 보기 — 행으로 드래그하면 그 티어에 들어가고, 티어 안 순서가 그대로 순위가 됩니다. 기본 5개, "+ Add Tier"로 추가(최대 10), ×로 삭제(아이템은 맨 아래 미분류 풀 "…"로 이동). 라벨 클릭으로 이름 변경 |
| 테마 전환 | 우상단 🌙 / ☀️ |
...
- 순위를 입력하면 그 위치에 끼워넣어집니다. 그 위치가 티어 안이면 해당 티어에 들어갑니다.
- 아이템 수보다 큰 수(예: 999)는 미분류 맨 뒤, 0 이하는 맨 앞(1위)으로 끼워넣어집니다.
