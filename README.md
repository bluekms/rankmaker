...
**그림 파일이 없어도 됩니다.** 설명에 이미지 주소를 적어 두면 그 주소로 보여주고, 주소도 없으면 이름만 적힌 카드가 나옵니다.
...
포스터에는 **그림 파일**이 필요합니다. 주소로만 있는 그림은 화면에는 잘 보이지만, 그 서버가 다른 사이트의 사용을 막고 있으면 포스터에 넣을 수 없습니다 — 브라우저 보안 규칙이라 우회할 방법이 없습니다.

이때는 안내창이 뜹니다. **그림을 복사해 `Paste` 를 누르면** 앱이 올바른 파일명으로 `topics/images/` 에 저장합니다. 직접 내려받아 넣어도 됩니다.
